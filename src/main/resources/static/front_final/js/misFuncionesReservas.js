function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://168.138.140.77:8080/api/Client/all",
        type:"GET",
        datatype:"JSON",
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://168.138.140.77:8080/api/Reservation/save",
            data: dataToSend,
            datatype: "json",
...
function listarReservation(){
    $.ajax({
        url:"http://168.138.140.77:8080/api/Reservation/all",
        type: "GET",
        datatype: "JSON",
        success: function (response) {
            console.log(response);
...
function pintarRespuestaReservation(response){
    ...
    myTable+="<tr>";
        ...
        myTable+="<td>fecha Cancelacion</td>";
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://168.138.140.77:8080/api/Reservation/"+idElemento,
            type: 'DELETE',
            contentType: "application/JSON",
            success: function (response) {
                console.log(response);
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://168.138.140.77:8080/api/Reservation/"+id,
        type: 'GET',
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://168.138.140.77:8080/api/Reservation/update",
            type: "PUT",
